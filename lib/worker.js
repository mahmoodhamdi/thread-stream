    ...
    if ((error.code === 'ENOTDIR' || error.code === 'ERR_MODULE_NOT_FOUND') &&
     filename.startsWith('file://')) {
      ...
      try {
        ...
        // Module resolution errors (MODULE_NOT_FOUND, ERR_REQUIRE_ESM, etc.)
        // mean the file couldn't be loaded via require — fall back to the
        // original import error. Runtime errors from the loaded module
        // (ReferenceError, TypeError, etc.) have no error code and should
        // bubble up to the user.
        // More info at: https://github.com/pinojs/thread-stream/issues/156
        if (requireError.code === 'MODULE_NOT_FOUND' ||
          (requireError.code && requireError.code.startsWith('ERR_'))) {
          throw error
        }
        throw requireError
      }
